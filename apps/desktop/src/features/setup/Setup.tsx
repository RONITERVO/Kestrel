...
import { ModelDownloader } from "../control/ModelDownloader";
import type { AppSnapshot, ControlSnapshot, SetupLocations, SetupProgress } from "../../contracts/index";
  ...
  const runInstall = async (component: string, acceptedIdeogramLicense = false, acceptedMuscriptorLicense = false) => {
    ...
    try {
      ...
      const next = await installSetupComponent({
        ...
        whisperCheckpointPath: component === "speech" ? existingModelPaths[WHISPER_MODEL_ID]?.trim() ?? "" : "",
  ...
  const installEssentials = async () => {
    ...
    try {
      ...
      for (const component of ["assistant", "wikipedia"]) {
        ...
        next = await installSetupComponent({
          component,
          installRoot: savedInstallRoot,
          wikipediaEdition: edition,
          acceptIdeogramNonCommercialLicense: false,
          whisperCheckpointPath: "",
