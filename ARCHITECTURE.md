...
The executable is one Tauri application with nine explicit native authorities:
...
Music projects use recoverable `music/<id>/project.json` as truth. Arrangement sections, song description, lyrics, and advanced generation settings remain editable producer data. Native code compiles those fields into the installed MiniMax Music 3 caption-and-lyrics contract and submits a code-owned ComfyUI graph; a selected local language model may propose either field but cannot render or apply its proposal. A successful 32 kHz stereo WAV is copied into the project before a take becomes complete, while prior takes remain immutable. Each take records the exact graph, resolved model filenames, prompt ID, seed, duration, size, and SHA-256. Startup marks unfinished projects and takes interrupted instead of resuming them. MuScriptor is a separate user-configured, fixed-argument audio-to-MIDI adapter because its gated non-commercial weights are not a music generator and cannot be bundled or silently downloaded.
