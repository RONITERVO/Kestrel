...
pub fn default_roots(extra: &[String], bonsai_root: &str) -> Vec<PathBuf> {
    ...
    if let Some(base) = directories::BaseDirs::new() {
        ...
        roots.extend([
            ...
            home.join("Kestrel Research")
                .join("model-downloads")
                .join("models"),
            home.join("Kestrel AI")
                .join("model-downloads")
                .join("models"),
...
mod tests {
    ...
    #[test]
    #[ignore = "requires an installed multimodal model on disk"]
    fn test_live_inspect_multimodal_models() {
        let roots = default_roots(&[], "");
        let models = scan(&roots);
        let mm = models
            .iter()
            .find(|m| m.supports_audio || m.supports_vision);
        if let Some(model) = mm {
            println!("Found multimodal model: {:#?}", model);
            if let Some(mmproj) = &model.mmproj_path {
                let meta = read_gguf_metadata(Path::new(mmproj)).unwrap();
                println!("MMProj metadata keys: {:?}", meta.keys().collect::<Vec<_>>());
            }
        } else {
            println!("No multimodal model found in default roots.");
        }
    }

    #[tokio::test]
    #[ignore = "requires installed multimodal model, mmproj, and llama-server"]
    async fn test_live_multimodal_visual_and_audio() {
        use base64::Engine as _;
        let roots = default_roots(&[], "");
        let models = scan(&roots);
        let mm = models
            .iter()
            .find(|m| (m.supports_audio || m.supports_vision) && m.mmproj_path.is_some());

        let (model_path, mmproj_path) = match mm {
            Some(m) => (PathBuf::from(&m.path), PathBuf::from(m.mmproj_path.as_ref().unwrap())),
            None => {
                println!("No multimodal model with mmproj found, skipping live test.");
                return;
            }
        };

        let engines = crate::runtime::detect_engines("", "");
        let engine_path = match engines.first() {
            Some(e) => e.path.clone(),
            None => {
                println!("No llama-server binary found, skipping live test.");
                return;
            }
        };

        println!("=== Starting llama-server with {} and MMProj ===", model_path.display());
        let port = portpicker::pick_unused_port().unwrap_or(18092);
            ...
            .spawn()
        {
            Ok(c) => c,
            Err(e) => {
                println!("Failed to spawn engine: {e}");
                return;
            }
        };
        ...
        for attempt in 1..=60 {
            ...
            if let Ok(res) = client.get(&health_url).send().await {
                if res.status().is_success() {
                    println!("llama-server became healthy after {} attempts ({}s)!", attempt, attempt as f64 * 0.5);
                    healthy = true;
                    break;
                }
            }
        ...
        if !healthy {
            println!("Server failed to become healthy in 30s. Stopping child...");
            let _ = child.kill();
            let _ = child.wait();
            return;
        }

        let completions_url = format!("http://127.0.0.1:{port}/v1/chat/completions");

        // 1. Test Visual Understanding
        println!("\n==========================================");
        println!("TEST 1: Visual Understanding with Image Input");
        println!("==========================================");

        let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let image_path = manifest_dir.join("icons/icon.png");
        if image_path.exists() {
                ...
                "messages": [
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Describe this image in detail. What object or symbol does it show, and what colors and stylistic elements are present?"
                            },
                            {
                                "type": "image_url",
                                "image_url": {
            ...
            println!("Sending visual completion request (image: {} bytes)...", img_bytes.len());
            let start = std::time::Instant::now();
            match client.post(&completions_url).json(&payload).send().await {
                Ok(response) => {
                    let status = response.status();
                    let body_text = response.text().await.unwrap_or_default();
                    println!("Visual response [HTTP {}] in {:.2}s:\n{}", status, start.elapsed().as_secs_f64(), body_text);
                }
                Err(err) => {
                    println!("Visual request failed: {:?}", err);
                }
            }
        } else {
            println!("Image path not found: {}", image_path.display());
        }

        // 2. Test Audio Understanding
        println!("\n==========================================");
        println!("TEST 2: Audio Understanding with Audio Input");
        println!("==========================================");

        // Synthesize or load a minimal valid WAV PCM
        let mut synth_wav = vec![
            b'R', b'I', b'F', b'F',
            0x24, 0x08, 0x00, 0x00, // Size
            b'W', b'A', b'V', b'E',
            b'f', b'm', b't', b' ',
            16, 0, 0, 0, // fmt chunk len
            1, 0, // PCM
            1, 0, // Mono
            0x80, 0x3E, 0, 0, // 16000 Hz
            0x00, 0x7D, 0, 0, // 32000 Bps
            2, 0, // Block align
            16, 0, // Bits per sample
            b'd', b'a', b't', b'a',
            0x00, 0x08, 0x00, 0x00, // 2048 data bytes
        ];
        synth_wav.resize(44 + 2048, 0);

        let b64_audio = base64::engine::general_purpose::STANDARD.encode(&synth_wav);
        let payload = serde_json::json!({
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "text",
                            "text": "Listen to this audio clip. Transcribe what is spoken, or describe what sounds and acoustic environment you hear."
                        },
                        {
                            "type": "input_audio",
                            "input_audio": {
        ...
        println!("Sending audio completion request (synthetic audio: {} bytes)...", synth_wav.len());
        let start = std::time::Instant::now();
        match client.post(&completions_url).json(&payload).send().await {
            Ok(response) => {
                let status = response.status();
                let body_text = response.text().await.unwrap_or_default();
                println!("Audio response [HTTP {}] in {:.2}s:\n{}", status, start.elapsed().as_secs_f64(), body_text);
            }
            Err(err) => {
                println!("Audio request failed: {:?}", err);
            }
        }
