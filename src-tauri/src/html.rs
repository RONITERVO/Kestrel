...
<meta name="generator" content="Kestrel Local {version}"><title>{title} · Kestrel Research</title>
<style>
...
<div class="layout"><main><div class="kicker">Research brief · Edition {edition} · {date}</div><h1>{title}</h1><p class="dek">{dek}</p><div class="byline"><span>{minutes} min read</span><span>{source_count} inspected sources</span><span>{words} words</span>{profile_meta}</div>
...
<script type="application/json" id="kestrel-report">{metadata}</script></body></html>"##,
        ...
        profile_meta = if report.research_profile == "solo-expedition" {
            format!(
                "<span>{} coordinated lanes</span><span>{} context / {} output</span>",
                report.research_lanes, report.context_window, report.output_budget
            )
        } else {
            String::new()
        },
