...
- **Setup** owns blank-Windows onboarding: one click installs the assistant and offline archive, while a second production-suite action installs verified movie finishing, H3, Music 3, Chatterbox narration, and the clearly labeled Whisper large-v3-turbo dictation download. Existing verified OpenAI Whisper checkpoints can be reused. Ideogram 4 remains a separate opt-in because its model agreement permits non-commercial use only; gated MuScriptor is a separate explicit import that prepares its isolated GPU runtime only after the producer accepts the official terms. Downloads are pinned, integrity-checked, observed, safely resumable, and kept on the producer-selected drive.
