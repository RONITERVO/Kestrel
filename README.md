...
Computer Tasks offer per-task **Approve for me** and **Approve manually** policies. Every state-changing or externally visible native action is validated first, then reviewed by a fresh compact request through the already-loaded model. The reviewer sees only the current task direction, native scope, and a bounded action envelope; large payloads are hashed and sampled at both ends. Its prompt and verdict never enter the main agent history, and it does not load another model or retain another conversation. Manual mode pauses on every proposed effect. Approve for me proceeds when scope and authorization are adequate, asks only for material ambiguity, and fails closed if review cannot complete. High-confidence destructive or persistent security-weakening commands are denied natively even after a mistaken approval. Rejections return one ordinary tool failure so the main agent can choose a materially safer path. Approval never expands the selected Workspace or Full Access boundary.
