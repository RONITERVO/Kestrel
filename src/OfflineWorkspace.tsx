...
export function OfflineWorkspace({ control, onChanged, onError }: Props) {
  ...
  return <div className="control-plane offline-workspace">
    ...
    <section className="control-center">
      ...
      </> : <ComputerTasks run={task} objective={objective} access={access} ready={control.runtime.phase === "ready"} running={!!taskRunRef.current} stopping={stoppingTask} fullUnlocked={settings.allowFullAccessAgent} onObjective={setObjective} onAccess={setAccess} onRun={() => void runTask()} onStop={() => void stopTask()} onOpen={(path) => task && void openTaskArtifact(task.id, path)} />}
    </section>
...
function ComputerTasks({ run, objective, access, ready, running, stopping, fullUnlocked, onObjective, onAccess, onRun, onStop, onOpen }: { run: ComputerTaskRun | null; objective: string; access: "workspace" | "full"; ready: boolean; running: boolean; stopping: boolean; fullUnlocked: boolean; onObjective: (value: string) => void; onAccess: (value: "workspace" | "full") => void; onRun: () => void; onStop: () => void; onOpen: (path: string) => void }) {
