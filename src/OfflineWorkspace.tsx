    ...
    <div className="control-plane offline-workspace">
      ...
      <aside className="control-inspector">
        ...
        <details className="inspector-section" open>
          ...
          <div className="inline-runtime-settings">
            <label>
              llama-server
              <input
                list="kestrel-engine-candidates"
                title={settings.enginePath}
                aria-invalid={!enginePathHasValidName}
                value={settings.enginePath}
                onChange={(event) =>
                  setSettings({ ...settings, enginePath: event.target.value })
                }
              />
              <datalist id="kestrel-engine-candidates">
                {control.engineCandidates.map((engine) => (
            ...
            <small className="engine-discovery-note">
              {!enginePathHasValidName
                ? "Choose a path ending in llama-server.exe"
                : control.engineCandidates.length
                  ? `${control.engineCandidates.length} verified local engine${control.engineCandidates.length === 1 ? "" : "s"} found`
                  : "No local engine found in Bonsai, Jan, or PATH"}
            </small>
            <div className="model-root-picker">
              {settings.extraModelRoots.map((root) => (
                <div key={root}>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setSettings((current) => ({
                        ...current,
                        extraModelRoots: current.extraModelRoots.filter(
                          (value) => value !== root,
                        ),
                      }))
                    }
              ...
              <small>
                Save, then rescan the model list. Matching vision/audio
                projectors are detected automatically.
              </small>
            </div>
            <ModelDownloader
        ...
        <details className="inspector-section" open={kind === "task"}>
          <summary>Computer Tasks policy</summary>
          <div className="inline-runtime-settings">
            <label>
              Maximum steps
              <input
...
function ModelDownloader({
  ...
  return (
    <section className="model-downloader wide" aria-label="Observed model downloader">
      <header>
        <span>
          <Download />
