...
import { cancelChatStream, deleteChatSession, getChatSession, getComputerTask, getControlSnapshot, listChatSessions, listComputerTasks, onChatStream, onComputerTaskEvent, onRuntimeProgress, openTaskArtifact, releaseAiMemory, resumeComputerTask, saveControlSettings, scanLocalModels, startChatStream, startComputerTask, startLocalModel, stopComputerTask, stopLocalModel } from "./api";
...
export function OfflineWorkspace({ control, onChanged, onError }: Props) {
  ...
  const [working, setWorking] = useState<"scan" | "start" | "stop" | "release" | "save" | "task" | null>(null);
  ...
  function handleChatEvent(event: ChatStreamEvent) {
    ...
    if (event.kind === "token" || event.kind === "reasoning") {
      ...
      setStream((current) => current ? { ...current, phase: event.kind === "done" ? "finishing" : event.kind } : current);
      ...
      void (async () => {
        try {
          ...
          if (terminal?.kind === "error") onError(terminal.content ?? "The local generation stopped unexpectedly.");
          const redirect = pendingRedirectRef.current;
          pendingRedirectRef.current = null;
          if (redirect) await launchChat(redirect, next);
        } catch (cause) {
          setStream(null);
          chatRequestRef.current = null;
          pendingRedirectRef.current = null;
  ...
  function handleTaskEvent(event: ComputerTaskEvent) {
    ...
    if (["done", "cancelled", "error", "limit", "question"].includes(event.kind)) {
  ...
  const newConversation = () => { if (stream) return; setSession(null); chatRequestRef.current = null; pendingRedirectRef.current = null; setKind("chat"); };
  const newTask = () => { if (active) return; setTask(null); setObjective(""); setTaskAnswer(""); setKind("task"); };

  async function launchChat(message: string, baseSession: ChatSession | null) {
    ...
    setSession(baseSession ? { ...baseSession, messages: [...baseSession.messages, optimistic] } : baseSession);
    try {
  ...
  const send = async () => {
    ...
    setDraft("");
    if (stream) {
      pendingRedirectRef.current = message;
      setStream((current) => current ? { ...current, phase: "redirecting" } : current);
      try { await cancelChatStream(stream.requestId); reconcileChatStop(stream.requestId); } catch (cause) { pendingRedirectRef.current = null; setDraft(message); onError(String(cause)); }
      return;
    }
    await launchChat(message, session);
  };

  const cancelGeneration = async () => {
    if (!stream) return;
    setStream((current) => current ? { ...current, phase: "stopping" } : current);
    try { await cancelChatStream(stream.requestId); reconcileChatStop(stream.requestId); } catch (cause) { onError(String(cause)); }
  };

  const continueGeneration = async () => {
    if (stream) return;
    await launchChat("Continue from the interrupted answer. Do not repeat completed material; first account for my latest instructions.", session);
  };
  ...
  const runTask = async () => {
    ...
    try {
      ...
      setObjective("");
      void refreshHistory();
    } catch (cause) { taskStartingRef.current = false; setWorking(null); onError(String(cause)); }
  };
  ...
  const resumeTask = async () => {
    ...
    if (task.access === "full" && !window.confirm("Continue this task with full computer access? The model may run programs and modify files outside workspace folders.")) return;
    setWorking("task");
    taskStartingRef.current = true;
    try {
      const next = await resumeComputerTask({ runId: task.id, answer: taskAnswer.trim() });
  ...
  const releaseMemory = async () => {
    if (!window.confirm("Release all AI memory controlled by Kestrel? This safely cancels active Kestrel work, stops the configured Bonsai model service, and removes abandoned Kestrel model processes. Other model apps are left alone.")) return;
    pendingRedirectRef.current = null;
    setWorking("release");
    try {
  ...
  const save = async () => {
    if (!enginePathHasValidName) {
      onError("The model engine path must end with llama-server.exe. Choose a verified local engine before saving.");
      return;
    }
    setWorking("save");
  ...
  const resumableAnswer = !stream && session?.messages.at(-1)?.role === "assistant" && ["interrupted", "limited"].includes(session.messages.at(-1)?.status ?? "");
  ...
  return <div className="control-plane offline-workspace">
    <aside className="model-drawer">
      ...
      <div className="control-models compact-models">{visibleModels.map((model) => <button key={model.id} disabled={active} className={selectedId === model.id ? "selected" : ""} onClick={() => { if (session && model.id !== session.modelId) newConversation(); setSelectedId(model.id); }}><Bot/><span><strong>{model.name}</strong><small>{model.source} · {model.quantization ?? "GGUF"}</small></span>{control.runtime.modelId === model.id && <i>{control.runtime.phase}</i>}</button>)}</div>
      <div className="drawer-title"><span>{kind === "chat" ? "CONVERSATIONS" : "TASK HISTORY"}</span>{kind === "chat" && <button title="New conversation" disabled={!!stream} onClick={newConversation}><MessageSquarePlus/></button>}</div>
      <div className="history-list">{kind === "chat" ? sessions.map((item) => <div key={item.id} className={session?.id === item.id ? "active" : ""}><button disabled={!!stream} onClick={() => void openSession(item)}><span>{item.title}</span><small>{item.messageCount} messages · {relativeTime(item.updatedAt)}</small></button><button title="Archive conversation" disabled={!!stream} onClick={() => void removeSession(item)}><Trash2/></button></div>) : tasks.map((item) => <button key={item.id} disabled={!!taskRunRef.current} className={task?.id === item.id ? "active" : ""} onClick={() => void openTask(item)}><span>{item.objective}</span><small>{item.status} · {item.eventCount} events · {relativeTime(item.updatedAt)}</small></button>)}</div>
    ...
    <section className="control-center">
      <header className="control-top"><div><span className="eyebrow">{kind === "chat" ? "LOCAL CONVERSATION" : "VISIBLE COMPUTER WORK"}</span><h1>{kind === "chat" ? session?.title ?? selected?.name ?? "Choose a model" : task?.objective ?? "Computer Tasks"}</h1></div><div className="control-actions">{kind === "chat" ? <button className="quiet-button" disabled={!!stream} onClick={newConversation}><MessageSquarePlus/> New chat</button> : <><button className="quiet-button" disabled={!!stream} onClick={newConversation}><MessageSquarePlus/> New chat</button><button className="quiet-button" disabled={active} onClick={newTask}><MessageSquarePlus/> New task</button></>}{control.runtime.phase === "ready" ? <button className="quiet-button" disabled={active} onClick={() => void act("stop", stopLocalModel)}>{working === "stop" ? <LoaderCircle className="spin"/> : <CircleStop/>}{control.runtime.mode === "attached" ? "Detach" : "Stop"}</button> : <button className="primary-button" disabled={!selected || !!working} onClick={() => selected && void act("start", () => startLocalModel(selected.id))}>{working === "start" ? <LoaderCircle className="spin"/> : <Play/>} Load model</button>}</div></header>
      {kind === "chat" ? <>
        ...
        {resumableAnswer && <div className="chat-resume"><span>The partial answer is saved.</span><button onClick={() => void continueGeneration()}><Play/> Continue answer</button></div>}
        <div className={`control-composer ${stream ? "steerable" : ""}`}><textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder={control.runtime.phase !== "ready" ? "Load a model to begin" : taskRunRef.current ? "Computer task is using the inference slot…" : stream ? "Add a message to redirect this answer…" : "Message the active local model…"} disabled={control.runtime.phase !== "ready" || !!taskRunRef.current}/>{stream && !draft.trim() ? <button title="Stop generation" className="stop-generation" onClick={() => void cancelGeneration()}><Square/></button> : <button title={stream ? "Send and redirect the current answer" : "Send"} onClick={() => void send()} disabled={!draft.trim() || control.runtime.phase !== "ready" || !!taskRunRef.current}>{stream ? <Zap/> : <Send/>}</button>}</div>
      </> : <ComputerTasks run={task} objective={objective} answer={taskAnswer} access={access} ready={control.runtime.phase === "ready"} running={!!taskRunRef.current} stopping={stoppingTask} resuming={working === "task"} fullUnlocked={settings.allowFullAccessAgent} onObjective={setObjective} onAnswer={setTaskAnswer} onAccess={setAccess} onRun={() => void runTask()} onResume={() => void resumeTask()} onStop={() => void stopTask()} onOpen={(path) => task && void openTaskArtifact(task.id, path)} />}
    </section>
    ...
    <aside className="control-inspector">
      ...
      <button className="release-memory" disabled={working === "release"} onClick={() => void releaseMemory()}>{working === "release" ? <LoaderCircle className="spin"/> : <CircleStop/>}<span><strong>Release AI memory</strong><small>Stop Kestrel + configured Bonsai models</small></span></button>
      <details className="inspector-section" open><summary>Model profile</summary><div className="inline-runtime-settings"><label>llama-server<input list="kestrel-engine-candidates" title={settings.enginePath} aria-invalid={!enginePathHasValidName} value={settings.enginePath} onChange={(event) => setSettings({...settings, enginePath:event.target.value})}/><datalist id="kestrel-engine-candidates">{control.engineCandidates.map((engine) => <option key={engine.path} value={engine.path}>{engine.source}</option>)}</datalist></label><small className="engine-discovery-note">{!enginePathHasValidName ? "Choose a path ending in llama-server.exe" : control.engineCandidates.length ? `${control.engineCandidates.length} verified local engine${control.engineCandidates.length === 1 ? "" : "s"} found` : "No local engine found in Bonsai, Jan, or PATH"}</small><label>Context<input type="number" disabled={!settings.advancedMode} value={settings.contextWindow} onChange={(event) => updatePositiveSetting("contextWindow", event.target.value)}/></label><label>Max chat output<input type="number" disabled={!settings.advancedMode} value={settings.maxOutputTokens} onChange={(event) => updatePositiveSetting("maxOutputTokens", event.target.value)}/></label><label className="check-line"><input type="checkbox" checked={settings.advancedMode} onChange={(event) => setSettings({...settings, advancedMode:event.target.checked})}/> Advanced, uncapped</label></div></details>
...
function ComputerTasks({ run, objective, answer, access, ready, running, stopping, resuming, fullUnlocked, onObjective, onAnswer, onAccess, onRun, onResume, onStop, onOpen }: { run: ComputerTaskRun | null; objective: string; answer: string; access: "workspace" | "full"; ready: boolean; running: boolean; stopping: boolean; resuming: boolean; fullUnlocked: boolean; onObjective: (value: string) => void; onAnswer: (value: string) => void; onAccess: (value: "workspace" | "full") => void; onRun: () => void; onResume: () => void; onStop: () => void; onOpen: (path: string) => void }) {
  const resumable = !!run && ["waiting", "cancelled", "interrupted", "failed"].includes(run.status);
  const question = run ? [...run.events].reverse().find((event) => event.kind === "question") : undefined;
  return <div className="computer-workspace">{!run || (!running && objective) ? <section className="task-launch"><div className="task-orbit"><MonitorCog/></div><span className="eyebrow">ACTUAL COMPUTER WORK</span><h2>Give the local model a bounded objective.</h2><p>Every decision, tool call, result, error, and artifact stays visible and is saved locally. Decision-critical questions pause safely for your answer. Workspace mode is the everyday default.</p><textarea value={objective} onChange={(event) => onObjective(event.target.value)} placeholder="Example: Check Downloads. If the newest file is not an SVG cat, create a polished cat.svg there and report its path."/><div className="task-policy"><button className={access === "workspace" ? "active" : ""} onClick={() => onAccess("workspace")}><ShieldCheck/><span><strong>Workspace</strong><small>Only approved folders</small></span></button><button className={access === "full" ? "active danger" : ""} disabled={!fullUnlocked} onClick={() => onAccess("full")}><Zap/><span><strong>Full access</strong><small>{fullUnlocked ? "Programs and all files" : "Locked in profile"}</small></span></button></div><button className="primary-button task-run" disabled={!ready || !objective.trim()} onClick={onRun}><Play/> Start visible task</button></section> : <section className="task-run-view"><header><div><span className="eyebrow">{run.access.toUpperCase()} ACCESS · {run.status.toUpperCase()}</span><h2>{run.objective}</h2></div>{running && <button className="danger-button" disabled={stopping} onClick={onStop}>{stopping ? <LoaderCircle className="spin"/> : <Square/>} {stopping ? "Stopping…" : "Stop safely"}</button>}</header><div className="task-timeline" aria-live="polite">{run.events.map((event, index) => <article key={`${event.at}-${index}`} className={`task-event ${event.kind}`}><div className="event-glyph">{event.kind === "artifact" ? <FileCode2/> : event.kind === "tool_start" ? <Wrench/> : event.kind === "thinking" || event.kind === "queued" ? <LoaderCircle className="spin"/> : event.kind === "done" ? <Check/> : <ChevronRight/>}</div><div><header><strong>{event.title}</strong><span>{event.step ? `Step ${event.step}` : "Setup"} · {timeOnly(event.at)}</span></header><pre>{event.detail}</pre>{event.kind === "artifact" && event.data?.path && <button className="artifact-button" onClick={() => onOpen(event.data!.path!)}><FolderOpen/> Open artifact</button>}</div></article>)}</div>{resumable && <section className="task-question"><span className="eyebrow">{run.status === "waiting" ? "YOUR DECISION" : "CONTINUE DURABLE TASK"}</span><h3>{question?.detail ?? "Add direction before the model resumes."}</h3>{question?.data?.options && <div className="question-options">{question.data.options.map((option, index) => <button key={option} className={question.data?.recommendedIndex === index ? "recommended" : ""} onClick={() => onAnswer(option)}>{option}{question.data?.recommendedIndex === index && <small>Recommended</small>}</button>)}</div>}<textarea value={answer} onChange={(event) => onAnswer(event.target.value)} placeholder="Answer or add a precise continuation instruction…"/><button className="primary-button" disabled={!ready || !answer.trim() || resuming} onClick={onResume}>{resuming ? <LoaderCircle className="spin"/> : <Play/>} Resume this task</button></section>}{run.artifacts.length > 0 && <footer className="artifact-shelf"><span><FileCode2/> {run.artifacts.length} artifact{run.artifacts.length === 1 ? "" : "s"}</span>{run.artifacts.map((path) => <button key={path} onClick={() => onOpen(path)} title={path}>{fileName(path)}</button>)}</footer>}</section>}</div>;
}

function Message({ message, model }: { message: ChatMessage; model?: string }) { return <article className={message.role}><span>{message.role === "user" ? "YOU" : model ?? "MODEL"}{message.status && <i>{message.status === "limited" ? "output limit · continuation available" : "interrupted · partial saved"}</i>}<button title="Copy message" onClick={() => void navigator.clipboard.writeText(message.content)}><Clipboard/></button></span>{message.reasoning && <details><summary>Reasoning</summary><pre>{message.reasoning}</pre></details>}<RichText value={message.content}/></article>; }
...
export function terminalTaskStatus(kind: string, fallback: string) { if (kind === "done") return "completed"; if (kind === "cancelled") return "cancelled"; if (kind === "question") return "waiting"; if (kind === "error" || kind === "limit") return "failed"; return fallback === "starting" ? "running" : fallback; }
