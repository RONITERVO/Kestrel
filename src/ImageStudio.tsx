...
export function ImageStudio({
  ...
  return <div className={`image-studio ${showLibrary ? "library-visible" : ""}`} tabIndex={-1} onKeyDown={handleStudioKeyDown}>
    ...
    {showLibrary && <aside className="image-library">
      <div className="image-pane-heading"><span><small>Private library</small><strong>Image projects</strong></span><button onClick={() => setNewOpen(true)}><Plus /></button></div>
...
function parsePalette(value: string, maximum: number): string[] {
  return value.split(/[\s,]+/).map((color) => color.trim().toUpperCase()).filter(Boolean).slice(0, maximum);
}
